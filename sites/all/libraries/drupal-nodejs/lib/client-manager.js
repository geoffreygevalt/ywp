  ...
  if (this.sockets[socketId] && message.hasOwnProperty('type')) {
    ...
    if (message.hasOwnProperty('channel')) {
      if (this.channelIsClientWritable(message.channel) && this.clientIsInChannel(socketId, message.channel)) {
        ...
        this.logger.debug("processMessage: Received unauthorised message from client: cannot write to channel " + socketId);
      }
    }
    ...
    // that this is allowed in the server configuration.
      ...
      this.logger.debug("processMessage: Received unauthorised message from client: cannot write to client " + socketId);
    }
  }
};
...
ClientManager.prototype.authenticateClientCallback = function (error, response, body) {
  ...
  if (response.statusCode == 404) {
    ...
    this.logger.debug("authenticateClientCallback: Response data", response);
    return false;
  }
...
ClientManager.prototype.cleanupSocket = function (socket) {
  ...
  for (var tokenChannel in this.tokenChannels) {
    this.logger.log("cleanupSocket: checking tokenChannel " + tokenChannel + " for socket " + socket.id);

    if (this.tokenChannels[tokenChannel].sockets[socket.id]) {
      this.logger.log("cleanupSocket: found socket info for tokenChannel " + tokenChannel, this.tokenChannels[tokenChannel].sockets[socket.id]);
